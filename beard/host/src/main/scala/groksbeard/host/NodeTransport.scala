...
final class NodeTransport(child: ChildProcessHandle, log: String => Unit) extends AcpTransport:
  private var listener: String => Unit = _ => ()
  child.stdout.setEncoding("utf8")
  child.stderr.setEncoding("utf8")
  child.stdout.on("data", (chunk: js.Any) => listener("" + chunk))
  child.stderr.on("data", (chunk: js.Any) => log("" + chunk))
...
object NodeTransport:
  def spawn(command: String, args: List[String], cwd: String, log: String => Unit): NodeTransport =
    val child = nodeChildProcess.spawn(
      command,
      js.Array(args*),
      js.Dynamic.literal(cwd = cwd, stdio = js.Array("pipe", "pipe", "pipe")),
    )
    new NodeTransport(child, log)
end NodeTransport
