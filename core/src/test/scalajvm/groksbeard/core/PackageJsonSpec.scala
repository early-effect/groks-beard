    ...
    suite("package.json")(
      test("contributes Add Selection, Add File, and the @-ref copy command") {
        ...
        assertTrue(
          ...
          json.contains("\"groksBeard.renameSession\""),
          json.contains("\"groksBeard.deleteSession\""),
          json.contains("\"groksBeard.keepAll\""),
          json.contains("\"groksBeard.undoAll\""),
          json.contains("\"groksBeard.keepTurn\""),
  ...
  private def packageJson: Path =
    ...
    Iterator
      ...
      .map { dir =>
        ...
        if Files.isRegularFile(here) && Files.readString(here).contains("groks-beard") then here
        else null
      }
      .find(_ != null)
      .getOrElse(sys.error(s"package.json not found from $start"))
  end packageJson
end PackageJsonSpec
